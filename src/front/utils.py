...
class E(object):
    ...
    def cost4search(xp, times):
        ...
        try:
            ...
            hunttimes = [D.HUNTTIMES[i*3+2] for i in xrange(0, len(D.HUNTTIMES)/3) if lv <= D.HUNTTIMES[i*3+1] and lv >= D.HUNTTIMES[i*3]]
    ...
    def earn4reclaim(user, guards, heros):
        try:
            ...
            gold, feat = [(D.HUNTRECLAIM[i*4+2], D.HUNTRECLAIM[i*4+3]) for i in xrange(0, len(D.HUNTRECLAIM)/4) if csword <= D.HUNTRECLAIM[i*4+1] and csword >= D.HUNTRECLAIM[i*4]][0]
            earn = dict(gold=gold, feat=feat)
        except Exception:
            earn = {'gold':0, 'feat':0}
        return earn
    ...
    def buy4hp(times):
        try:
            rock, hp = [(D.HPBUY[i*2+1], D.HPBUY[i*2+2]) for i in xrange(0, len(D.HPBUY)/3) if times == D.HPBUY[i*2]][0]
